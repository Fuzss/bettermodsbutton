...
//        setupConfig(this.display, builder, saveCallback);
    ...
    public static class GeneralConfig extends AbstractConfig {
        ...
        public static ModScreenHandler.MainMenuMode mainMenuMode = ModScreenHandler.MainMenuMode.INSERT_BELOW_REALMS;
        @Config(description = "Add mod count to mods button.")
        public static boolean modCount = true;
        @Config(name = "pause_screen_mods_button", description = "Where to place mods button on pause menu screen.")
        public static ModScreenHandler.PauseScreenMode pauseScreenMode = ModScreenHandler.PauseScreenMode.INSERT_BELOW_FEEDBACK_AND_BUGS;
        ...
        protected void addToBuilder(ForgeConfigSpec.Builder builder, ConfigHolder.ConfigCallback saveCallback) {
            ...
            saveCallback.accept(builder.comment("Force-run Pickup Notifier on the client-side only.", "Only enable this when there are problems, e. g. the same pick-up being logged multiple times. Partial item stack pick-ups (when the inventory is full) won't show, and backpack compat won't work.", "When playing on a server without this mod this option will be used automatically (for technical reasons).").define("Force Client Only", false), v -> this.forceClient = v);
